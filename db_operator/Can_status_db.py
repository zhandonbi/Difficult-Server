...
class CanStatusDb(object):
    ...
    def Can_status_add(self, Can_ID: str, GK152_state: str, GK152_data: str, HCSR04_state: str, HCSR04_distance,
                       LEDBuzzer_1: str, LEDBuzzer_2: str, Time: str):
        '''
        向数据库中添加垃圾桶运行的状态
        :param Can_ID: 垃圾桶编号
        ...
        sql = 'insert into Can_status(Can_ID,GK152_state,GK152_data,HCSR04_state,HCSR04_distance,LEDBuzzer_1,LEDBuzzer_2,Time) values("{0}","{1}","{2}","{3}","{4}","{5}","{6}","{7}") '.format(
            Can_ID, GK152_state, GK152_data, HCSR04_state, HCSR04_distance, LEDBuzzer_1, LEDBuzzer_2, Time)
    ...
    def Can_status_search(self, Can_ID: str):
        ...
        if len(search_results) != 0:
            ...
            result['Time'] = search_results[len(search_results) - 1][7]
        else:
            return '不存在该设备ID的垃圾桶或该垃圾桶尚未开始工作'
        return result
